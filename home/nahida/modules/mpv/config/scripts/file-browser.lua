...
package.path = mp.command_native({ "expand-path", o.module_directory })
  .. "/?.lua;"
  .. package.path
...
local parser_API = setmetatable(
  {},
  { __index = package.loaded["file-browser"] }
)
...
local style = {
  ...
  body = ([[{\r\q2\fs%d\fn%s\c&H%s&}]]):format(
    o.font_size_body,
    o.font_name_body,
    o.font_colour_body
  ),
  ...
  playing_selected = ([[{\c&H%s&}]]):format(
    o.font_colour_playing_multiselected
  ),

  --icon styles
  cursor = ([[{\fn%s\c&H%s&}]]):format(
    o.font_name_cursor,
    o.font_colour_cursor
  ),
  cursor_select = ([[{\fn%s\c&H%s&}]]):format(
    o.font_name_cursor,
    o.font_colour_multiselect
  ),
  cursor_deselect = ([[{\fn%s\c&H%s&}]]):format(
    o.font_name_cursor,
    o.font_colour_selected
  ),
...
function API.coroutine.assert(err)
  ...
  assert(
    not main and co,
    err or "error - function must be executed from within a coroutine"
  )
  return co
end
...
function API.coroutine.callback()
  local co = API.coroutine.assert(
    "cannot create a coroutine callback for the main thread"
  )
  return function(...)
...
function API.join_path(working, relative)
  return API.get_protocol(relative) and relative
    or utils.join_path(working, relative)
end
...
function API.sort(t)
  ...
  for i, f in ipairs(t) do
    tuples[i] = {
      f.type:sub(1, 1)
        .. (f.label or f.name):lower():gsub("0*(%d+)%.?(%d*)", padnum),
      f,
    }
  end
...
function API.parseable_item(item)
  return item.type == "dir"
    or parseable_extensions[API.get_extension(item.name, "")]
end
...
function API.iterate_opt(str)
  return string.gmatch(
    str,
    "([^" .. API.pattern_escape(o.root_separators) .. "]+)"
  )
end
...
local function json_safe_recursive(t)
  ...
  for key, value in pairs(t) do
    ...
    if
      ...
      and vtype ~= "thread"
      and (
        (isarray and ktype == "number" and key <= array_length)
...
local function update_current_directory(_, filepath)
  ...
  if filepath == nil then
    current_file.directory =
      API.fix_path(mp.get_property("working-directory", ""), true)
...
local function update_ass()
  ...
  append(style.header)
  append(
    API.ass_escape(dir_name, style.cursor .. "\\\239\187\191n" .. style.header)
  )
  append("\\N ----------------------------------------------------")
  newline()
...
local function select_prev_directory()
  if state.prev_directory:find(state.directory, 1, true) == 1 then
    ...
    while state.list[i] and API.parseable_item(state.list[i]) do
      if
        state.prev_directory:find(API.get_full_path(state.list[i]), 1, true)
      then
        state.selected = i
        return
      end
...
local function choose_and_parse(directory, index)
  ...
  while
    list == nil
    and not parse_state.already_deferred
    and index <= #parsers
  do
...
local function parse_directory(directory, parse_state)
  local co = API.coroutine.assert(
    "scan_directory must be executed from within a coroutine - aborting scan "
...
local function update_list(moving_adjacent)
  ...
  if not opts.escaped then
    for i = 1, #list do
      list[i].ass = list[i].ass
        or API.ass_escape(list[i].label or list[i].name, true)
    end
  end
...
local function down_dir()
  ...
  --we can make some assumptions about the next directory label when moving up or down
  if state.directory_label then
    state.directory_label = state.directory_label
      .. (current.label or current.name)
  end
...
local function concurrent_loadlist_parse(
  directory,
  load_opts,
  prev_dirs,
  item_t
...
local function open_item(item, opts)
  ...
  elseif audio_extensions[API.get_extension(item.name, "")] then
    mp.commandv(
      "audio-add",
      path,
      opts.flag == "replace" and "select" or "auto"
    )
  else
...
local function create_item_string(fn)
  ...
  return function(cmd, items, state, code)
    ...
    for i = 2, #items, 1 do
      str = str
        .. (cmd["concat-string"] or " ")
        .. func(cmd, items[i], state, code)
    end
    return str
  end
end
...
local KEYBIND_CODE_PATTERN = ("%%%%([%s])"):format(
  API.ass_escape(CUSTOM_KEYBIND_CODES)
)
...
local function run_custom_command(cmd, items, state)
  local custom_cmds = cmd.codes and format_command_table(cmd, items, state)
    or cmd.command
...
local function run_custom_keybind(cmd, state, co)
  ...
  if cmd.parser then
    ...
    if
      not parser_str:find(
        "%W" .. (state.parser.keybind_name or state.parser.name) .. "%W"
      )
    then
      return false
    end
  ...
  if not (cmd.multiselect and next(state.selection)) then
    ...
    if
      do_item_condition
      and not evaluate_condition(cmd.condition, { state.list[state.selected] })
    then
      return false
    end
  ...
  local selection = API.sort_keys(state.selection, function(item)
    if
      do_item_condition
      and not evaluate_condition(cmd.condition, { item })
    then
      return false
    end
...
local function insert_custom_keybind(keybind)
  ...
  if
    type(keybind.command) == "table" and type(keybind.command[1]) ~= "table"
  then
...
local function setup_keybinds()
    ...
    for i = #parsers, 1, -1 do
      ...
      if parser.keybinds then
        for i, keybind in ipairs(parser.keybinds) do
          ...
          keybind.name = parsers[parser].id
            .. "/"
            .. (keybind.name or tostring(i))
          keybind.addon = true
  ...
  if o.custom_keybinds then
    local path = mp.command_native({ "expand-path", "~~/script-opts" })
...
function parser_API:register_root_item(item, priority)
  return API.register_root_item(
    item,
    priority or parsers[self:get_id()].priority
  )
end
...
function parse_state_API:yield(...)
  ...
  if self.source == "browser" and not is_browser then
    msg.error(
      "current coroutine does not match browser's expected coroutine - did you unsafely yield before this?"
    )
    error(
      "current coroutine does not match browser's expected coroutine - aborting the parse"
    )
  end
  ...
  if is_browser and co ~= state.co then
    msg.verbose(
      "browser no longer waiting for list - aborting parse for",
      self.directory
    )
    error(ABORT_ERROR)
  end
...
local function load_addon(path)
  ...
  addon_environment.package.loaded =
    API.redirect_table(addon_environment.package.loaded)
  local msg_module = {
    log = function(level, ...)
...
local function setup_parser(parser, file)
  ...
  parser.name = parser.name
    or file:gsub("%-browser%.lua$", ""):gsub("%.lua$", "")
...
local function setup_addons()
  local addon_dir =
    mp.command_native({ "expand-path", o.addon_directory .. "/" })
...
mp.register_script_message("delay-command", function(delay, ...)
  ...
  local success, err = pcall(
    mp.add_timeout,
    API.evaluate_string("return " .. delay),
    function()
      mp.commandv(table.unpack(command))
    end
  )
...
mp.register_script_message(
  ...
  function(directory, response_str)
