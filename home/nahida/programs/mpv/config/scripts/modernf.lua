...
local osc_styles = {
  ...
  Tooltip = "{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H000000&\\fs18\\fn"
    .. user_opts.font
    .. "}",
  Title = "{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs36\\q2\\fn"
    .. user_opts.font
    .. "}",
...
local nicetypes =
  { video = texts.video, audio = texts.audio, sub = texts.subtitle }
...
layouts["reduced"] = function()
  ...
  add_area(
    "showhide",
    0,
    osc_param.playresy - user_opts.deadzone,
    osc_param.playresx,
    osc_param.playresy
  )
  ...
  lo.geometry =
    { x = osc_geo.w / 2, y = refY - 96, an = 5, w = osc_geo.w - 50, h = 2 }
  lo.layer = 13
...
function validate_user_opts()
  ...
  then
    msg.warn(
      "windowcontrols cannot be '" .. user_opts.windowcontrols .. "'. Ignoring."
    )
    user_opts.windowcontrols = "auto"
  end
end
...
function osc_init()
  ...
  ne.eventresponder["mbtn_right_up"] = function()
    mp.commandv("script-binding", "recent/display-recent")
  end
...
function render()
  ...
  -- submit
  set_osd(
    osc_param.playresy * osc_param.display_aspect,
    osc_param.playresy,
    ass.text,
    1000
  )
...
function process_event(source, what)
  ...
  elseif what == "up" then
    if elements[state.active_element] then
      ...
      elseif
        element_has_action(elements[n], action)
        and mouse_hit(elements[n])
      then
        elements[n].eventresponder[action](elements[n])
      end
