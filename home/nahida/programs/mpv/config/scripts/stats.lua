...
local function has_vo_window()
  return mp.get_property_native("vo-configured")
    and mp.get_property_native("video-osd")
end
...
local function append_perfdata(s, dedicated_page, print_passes)
  ...
  s[#s + 1] = format(
    ...
    o.font_size * 0.66,
    "(上一/平均/最高 μs)",
    dedicated_page and " (提示: 可用上下方向键滚动)" or "",
    o.font_size
  )
  ...
  for _, frame in ipairs(sorted_keys(vo_p)) do -- ensure fixed display order
    ...
    if print_passes then
      s[#s + 1] =
        format("%s%s%s:", o.nl, o.indent, b(frame:gsub("^%l", string.upper)))
...
local function get_kbinfo_lines(width)
  ...
  local cutoff = term
    and (width or 79) - o.indent:len() - itabs * 7 - spre:len()
...
local function append_display_sync(s)
  ...
  if
    not display_timer.oneshot
    and (o.plot_vsync_ratio or o.plot_vsync_jitter)
    and o.use_ass
  then
    local ratio_graph = ""
    local jitter_graph = ""
...
local function add_file(s)
  ...
  if
    not (mp.get_property_osd("filename") == mp.get_property_osd("media-title"))
  then
  ...
  if ed_cond then
    append_property(
      s,
      "edition-list/" .. tostring(edition) .. "/title",
      { prefix = "版本:" }
    )
...
local function append_resolution(s, r, prefix, w_prop, h_prop, video_res)
  ...
  if append(s, r[w_prop], { prefix = prefix }) then
    ...
    if
      r["crop-w"]
      and (not video_res or not crop_noop(r[w_prop], r[h_prop], r))
    then
      append(
        s,
        format(
...
local function append_hdr(s, hdr, video_out)
  ...
  then
    ...
    append(
      ...
      format(
        "%.1f / %.1f / %.1f",
        hdr["scene-max-r"] or 0,
        hdr["scene-max-g"] or 0,
        hdr["scene-max-b"] or 0
      ),
...
local function append_img_params(s, r, ro)
  ...
  if ro and (r["w"] ~= ro["dw"] or r["h"] ~= ro["dh"]) then
    if
      ro["crop-w"]
      and (crop_noop(r["w"], r["h"], ro) or crop_equal(r, ro))
    then
      ro["crop-w"] = nil
    end
...
local function append_fps(s, prop, eprop)
  ...
  if
    fps ~= ""
    and append(s, fps, { prefix = prefix, suffix = unit .. suffix })
  then
    prefix = ""
    nl = ""
    indent = ""
...
local function add_video_out(s)
  ...
  if not r then
    ...
    append_resolution(
      s,
      { w = scaled_width, h = scaled_height, s = scale },
      "分辨率:"
    )
    return
  end
...
local function add_video(s)
  ...
  if
    append_property(s, "video-codec", { prefix_sep = "", nl = "", indent = "" })
  then
...
local function cache_stats()
  ...
  append(
    ...
    { prefix = "速度:", suffix = speed_graph }
  )

  append(
    stats,
    utils.format_bytes_humanized(info["total-bytes"]),
    { prefix = "总占用内存:" }
  )
  append(
    stats,
    utils.format_bytes_humanized(info["fw-bytes"]),
    { prefix = "前段占用内存:" }
  )
  ...
  append(
    stats,
    "",
    { prefix = o.nl .. o.nl .. "缓存范围:", nl = "", indent = "" }
  )

  append(
    stats,
    info["bof-cached"] and "yes" or "no",
    { prefix = "开头缓存:" }
  )
  append(
    stats,
    info["eof-cached"] and "yes" or "no",
    { prefix = "末尾缓存:" }
  )
