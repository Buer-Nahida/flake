...
options.read_options(o, nil, function(list)
  split_option_exts(
    list.additional_video_exts,
    list.additional_audio_exts,
...
function validate_directory_mode()
  if
    o.directory_mode ~= "recursive"
    and o.directory_mode ~= "lazy"
    and o.directory_mode ~= "ignore"
  then
...
function find_and_add_entries()
  ...
  if
    ...
    or (pl_count == 1 and EXTENSIONS[string.lower(this_ext)] == nil)
  then
  ...
  local files = {}
  do
    local dir_mode = o.directory_mode
      or mp.get_property("directory-mode", "lazy")
    local separator = mp.get_property_native("platform") == "windows" and "\\"
      or "/"
